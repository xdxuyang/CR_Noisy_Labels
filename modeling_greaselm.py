...
class GreaseLM(nn.Module):
    ...
    def forward(self, *inputs, cache_output=False, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type = _inputs
        ...
        logits,logits_1,logits_2 = self.lmgnn(lm_inputs, concept_ids,
        ...
        logits_1 = logits_1.view(bs, nc)
        logits_2 = logits_2.view(bs, nc)
        if not detail:
            return logits,logits_1,logits_2
        else:
            return logits,logits_1, logits_2,concept_ids.view(bs, nc, -1), node_type_ids.view(bs, nc, -1), edge_index_orig, edge_type_orig
...
class LMGNN(nn.Module):
    ...
    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        config, _ = ModelClass.config_class.from_pretrained(
            pretrained_model_name_or_path='/home/dell/.cache/huggingface/transformers/art/',
        ...
        self.fc_2 = layers.MLP(concat_vec_dim, 512, 1, n_fc_layer+1, p_fc, layer_norm=True)
        # self.fc_3 = layers.MLP(512, fc_dim, 1, n_fc_layer, p_fc, layer_norm=True)
        self.dropout_e = nn.Dropout(p_emb)
        self.dropout_fc = nn.Dropout(p_fc)
        self.dropout_e_1 = nn.Dropout(0.5)
        if init_range > 0:
            self.apply(self._init_weights)

        self.mp, self.loading_info = TextKGMessagePassing.from_pretrained('/home/dell/.cache/huggingface/transformers/large/', output_hidden_states=True, output_loading_info=True, args=args, k=k, n_ntype=n_ntype, n_etype=n_etype, dropout=p_gnn, concept_dim=concept_dim, ie_dim=ie_dim, p_fc=p_fc, info_exchange=info_exchange, ie_layer_num=ie_layer_num, sep_ie_layers=sep_ie_layers)
    ...
    def drop_tokens(self,length,word_drop):
        batch,dim= length.size()
        mask = length.new_empty(batch,dim)
        mask=mask.bernoulli_(1-word_drop)
        length_m=length*mask

        return length_m

    def drop_kg(self,length,word_drop):
        batch,node,dim= length.size()
        mask = length.new_empty(batch,node)
        mask=mask.bernoulli_(1-word_drop)
        length_m=length*mask.unsqueeze(-1).expand_as(length).float()
    ...
    def forward(self, inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, emb_data=None, cache_output=False):
        ...
        # input_ids_1 = self.drop_tokens(input_ids,0.05)
        ...
        gnn_input_1 =self.drop_kg(gnn_input,0.5)

        gnn_input[:, 0] = 0
        gnn_input_1[:, 0] = 0

        gnn_input = self.dropout_e(gnn_input) #(batch_size, n_node, dim_node)
        gnn_input_1 = self.dropout_e(gnn_input_1)
        # gnn_input_1 = self.dropout_e_1(gnn_input)
        ...
        outputs_1, gnn_output_1 = self.mp(input_ids, token_type_ids, attention_mask, output_mask, gnn_input_1,adj, node_type_ids, node_scores, special_nodes_mask, output_hidden_states=True)
        ...
        sent_vecs_for_pooler_1 = sent_vecs_1
        graph_vecs, pool_attn = self.pooler(sent_vecs_for_pooler, gnn_output, mask)
        graph_vecs_1, pool_attn_1 = self.pooler(sent_vecs_for_pooler_1, gnn_output_1, mask)
        ...
        concat_1 = torch.cat((graph_vecs_1, sent_vecs_1, Z_vecs_1), 1)



        logits = self.fc(self.dropout_fc(concat))
        logits_1 = self.fc(self.dropout_fc(concat_1))
        logits_2 =self.fc_2(self.dropout_fc(concat))
...
def test_TextKGMessagePassing(device):
    model = TextKGMessagePassing.from_pretrained('/home/dell/.cache/huggingface/transformers/large/', output_hidden_states=True).to(device)
    inputs = model.get_fake_inputs(device)
    outputs = model(*inputs)
    model.check_outputs(*outputs)
...
def test_RoBERTaGAT(device):
    config, _ = modeling_roberta.RobertaModel.config_class.from_pretrained(
        '/home/dell/.cache/huggingface/transformers/large/',
        cache_dir=None, return_unused_kwargs=True,
        force_download=False,
        output_hidden_states=True
    )
