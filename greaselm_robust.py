...
def calc_loss_and_acc(logits, labels, loss_type, loss_func):
    ...
    n_corrects = (logits.argmax(1) == labels.argmax(1)).sum().item()

    return loss, n_corrects


def TargetM(y_t,devices):
    f = y_t
    cap_m = (f == 0).sum()
    if cap_m == 4:
        cls_m =0
        ...
        cls_m = f[f > 0].max()
    f[f == 0] = torch.arange(0, cap_m,device=devices[1])+ cls_m + 1
    return f.view(-1, 1) == f.view(1, -1)
def SoftCE(s, t):
    m_t=nn.Softmax(dim=-1)
    s = m_t(s)
    loss_t = - (t * torch.log(s)).sum(dim=-1)
    return (loss_t / t.sum(dim=-1)).mean()

def calc_eval_accuracy(eval_set, model, loss_type, loss_func, debug, save_test_preds, preds_path,debices):
    ...
    x1 = torch.zeros(8500, 5)
    x2 = torch.zeros(8500, 5)
    x3 = torch.zeros(8500, 5)
    y = torch.zeros(8500)
    i=0
    with torch.no_grad():
        for qids, labels,true_labels, *input_data in tqdm(eval_set, desc="Dev/Test batch"):
            bs = true_labels.size(0)
            logits,logits1,logits2 = model(*input_data)
            x1[i:i+bs,:]=logits
            x2[i:i+bs,:]=logits1
            x3[i:i + bs, :] = logits2
            y[i:i+bs] = true_labels
            i = i+bs
            N, C = logits.shape
            given_labels = torch.full(size=(N, C), fill_value=0).to(debices[0])
            given_labels.scatter_(dim=1, index=torch.unsqueeze(true_labels, dim=1), value=1)
            loss, n_corrects = calc_loss_and_acc(logits, given_labels, loss_type, loss_func)
            loss = loss.mean()
            ...
            # if debug:
            #     break
        scio.savemat('flip0.4_our.mat', {'x1': x1.data.cpu().numpy(),'x2': x2.data.cpu().numpy(),'x3': x3.data.cpu().numpy(), 'y': y.data.cpu().numpy()})
...
def lrt_correction(y_tilde, f_x, current_delta=0.3, delta_increment=0.1):
    ...
    if corrected_count < 0.001 * n:
        ...
        print("Update Critical Value -> {}".format(current_delta), "red")
...
def train(args, resume, has_test_split, devices, kg):
    ...
    C = uniform_mix_C(0.2, 5)
    # C1 = flip_labels_C(0.2, 4)
    for i in range(8500):
        dataset.train_labels[dataset.inhouse_train_indexes[i]] = np.random.choice(5, p=C[dataset.train_labels[dataset.inhouse_train_indexes[i]]])
    # for i in range(4957):
    #     dataset.train_labels[i] = np.random.choice(4, p=C1[dataset.train_labels[i]])
    ...
    if args.loss == 'margin_rank':
        loss_func = nn.MarginRankingLoss(margin=0.1, reduction='none')
    ...
    print()
    print('-' * 71)
    norm = modeling_greaselm.pNorm(1)
    total_loss_acm = 0.0
    n_samples_acm = n_corrects_acm = 0
    total_time = 0
    model.train()
    ...
    for epoch_id in trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        if epoch_id < args.unfreeze_epoch:
            threshold_clean = tau_clean
        else:
            threshold_clean = min((tau_c_max - tau_clean) * (epoch_id+1 - args.unfreeze_epoch) / (args.n_epochs - args.unfreeze_epoch) + tau_clean,0.8)
        ...
        if epoch_id == args.refreeze_epoch:
            utils.freeze_params(params_to_freeze)
        model.train()
        print(threshold_clean)
        sum_correct =0
        num_correct =0
        s_correct =0
        n_correct =0
        for qids, labels,true_labels, *input_data in tqdm(train_dataloader, desc="Batch"):
            # labels: [bs]
            ...
            for a in range(0, bs, args.mini_batch_size):
                ...
                logits,logits_1,logits_2 = model(*[x[a:b] for x in input_data])
                probs1 = F.softmax(logits, dim=1)
                probs2 = F.softmax(logits_1, dim=1)
                probs3 = F.softmax(logits_2, dim=1)
                N, C = logits.shape
                given_labels = torch.full(size=(N, C), fill_value=eps / (C - 1)).to(devices[0])
                given_labels.scatter_(dim=1, index=torch.unsqueeze(labels[a:b], dim=1), value=1 - eps)
                prob_clean = 1 - js_div(0.5*(probs1+probs2), given_labels)
                prob_clean_1 = 1 - js_div(0.5 * (probs1+probs3), given_labels)

                c = (probs1 +probs2 +probs3)/3
                agree = (prob_clean>= threshold_clean)
                agree_1 = (prob_clean_1>= threshold_clean)
                unclean = (prob_clean < threshold_clean)
                unclean_1 = (prob_clean_1 < threshold_clean)

                idx_agree = (agree * agree_1).nonzero(as_tuple=False).squeeze(dim=1)
                idx_ood_1 = (agree * unclean_1).nonzero(as_tuple=False).squeeze(dim=1)
                idx_ood_2 = (agree_1 * unclean).nonzero(as_tuple=False).squeeze(dim=1)
                idx_disagree = (unclean_1 * unclean).nonzero(as_tuple=False).squeeze(dim=1)

                n_correct = n_correct + (true_labels[a:b][idx_agree] == labels[a:b][idx_agree]).sum().item()
                s_correct = s_correct + (labels[a:b][idx_agree] == labels[a:b][idx_agree]).sum().item()


                given_labels[idx_ood_1] = F.softmax((given_labels[idx_ood_1]+c.clone().detach()[idx_ood_1]), dim=1)
                given_labels[idx_ood_2] = F.softmax((given_labels[idx_ood_2]+c.clone().detach()[idx_ood_2]), dim=1)
                Targets = c.clone().detach()[idx_disagree]+0.5*torch.randn((c.clone().detach()[idx_disagree]).size()).to(devices[0])
                given_labels[idx_disagree] =F.softmax(Targets,dim=1)




                loss0, n_corrects = calc_loss_and_acc(logits, given_labels, args.loss, loss_func)
                loss1, _ = calc_loss_and_acc(logits_1, given_labels, args.loss, loss_func)
                loss2, _ = calc_loss_and_acc(logits_2, given_labels, args.loss, loss_func)

                loss_0 = loss0[idx_agree].mean()+loss1[idx_agree].mean()+loss2[idx_agree].mean()
                total_loss_acm += (loss_0.item())


                loss_1 = loss0[idx_ood_1].mean()+loss0[idx_ood_2].mean()+loss1[idx_ood_1].mean()+loss1[idx_ood_2].mean()+loss2[idx_ood_1].mean()+loss2[idx_ood_2].mean()
                loss_2 = loss0[idx_disagree].mean()+loss1[idx_disagree].mean()+loss2[idx_disagree].mean()
                sign = torch.ones(N).to(devices[0])
                sign[idx_disagree] *= -0.5
                losses_o = symmetric_kl_div(probs1, probs2) * sign+symmetric_kl_div(probs1, probs3) * sign
                loss_o = losses_o.mean()

                loss = (1* loss_0 + 1*loss_1 + 1*loss_2 +1*loss_o + 1* norm(logits, p=None)) / bs
                num_correct =num_correct+(true_labels[a:b][idx_ood_1] == labels[a:b][idx_ood_1]).sum().item()+(true_labels[a:b][idx_ood_2] == labels[a:b][idx_ood_2]).sum().item()
                sum_correct =sum_correct+(labels[a:b][idx_ood_1] == labels[a:b][idx_ood_1]).sum().item()+(labels[a:b][idx_ood_2] == labels[a:b][idx_ood_2]).sum().item()
                loss.backward()
                n_corrects_acm += n_corrects
                n_samples_acm += (b - a)
        ...
        dev_total_loss, dev_acc = calc_eval_accuracy(dev_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path,devices)
        if has_test_split:
            preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
            test_total_loss, test_acc = calc_eval_accuracy(test_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path,devices)
        else:
            test_acc = 0

        print('-' * 71)
        print('| epoch {:3} | step {:5} | dev_acc {:7.4f} | test_acc {:7.4f} |num_correct {:3} | num_sum {:3} | n_correct {:3} | n_sum {:3} |'.format(epoch_id, global_step, dev_acc, test_acc,num_correct,sum_correct,n_correct,s_correct))
        print('-' * 71)
...
def evaluate(args, has_test_split, devices, kg):
    ...
    preds_path = os.path.join(args.save_dir, 'dev_e{}_preds.csv'.format(epoch_id))
    # dev_total_loss, dev_acc = calc_eval_accuracy(dev_dataloader, model, args.loss, loss_func, debug, not debug, preds_path,devices)
    if has_test_split:
        # Evaluation on the test set
        preds_path = os.path.join(args.save_dir, 'test_e{}_preds.csv'.format(epoch_id))
        test_total_loss, test_acc = calc_eval_accuracy(test_dataloader, model, args.loss, loss_func, debug, not debug, preds_path,devices)
    else:
        test_acc = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', default=save_dir, help='model output directory')
    parser.add_argument('--save_model', default=save_model, type=utils.bool_flag, help="Whether to save model checkpoints or not.")
    parser.add_argument('--load_model_path', default=load_model_path, help="The model checkpoint to load in the evaluation mode.")
    parser.add_argument('-h', '--help', action='help', default=argparse.SUPPRESS, help='show this help message and exit')
    parser.add_argument("--run_name", default=run_name, type=str, help="The name of this experiment run.")
    parser.add_argument("--resume_checkpoint", default=resume_checkpoint, type=str,
                        help="The checkpoint to resume training from.")
    parser.add_argument('--use_wandb', default=use_wandb, type=utils.bool_flag, help="Whether to use wandb or not.")
    parser.add_argument("--resume_id", default=resume_id, type=str, help="The wandb run id to resume if `resume_checkpoint` is not None or 'None'.")

    # Data
    parser.add_argument('--train_adj', default=train_adj, help="The path to the retrieved KG subgraphs of the training set.")
    parser.add_argument('--dev_adj', default=dev_adj, help="The path to the retrieved KG subgraphs of the dev set.")
    parser.add_argument('--test_adj', default=test_adj, help="The path to the retrieved KG subgraphs of the test set.")
    parser.add_argument('--max_node_num', default=max_node_num, type=int, help="Max number of nodes / the threshold used to prune nodes.")
    parser.add_argument('--subsample', default=subsample, type=float, help="The ratio to subsample the training set.")
    parser.add_argument('--n_train', default=-n_train, type=int, help="Number of training examples to use. Setting it to -1 means using the `subsample` argument to determine the training set size instead; otherwise it will override the `subsample` argument.")

    # Model architecture
    parser.add_argument('-k', '--k', default=k, type=int, help='The number of GreaseLM layers')
    parser.add_argument('--att_head_num', default=att_head_num, type=int, help='number of attention heads of the final graph nodes\' pooling')
    parser.add_argument('--gnn_dim', default=gnn_dim, type=int, help='dimension of the GNN layers')
    parser.add_argument('--fc_dim', default=fc_dim, type=int, help='number of FC hidden units (except for the MInt operators)')
    parser.add_argument('--fc_layer_num', default=fc_layer_num, type=int, help='number of hidden layers of the final MLP')
    parser.add_argument('--freeze_ent_emb', default=freeze_ent_emb, type=utils.bool_flag, nargs='?', const=True, help='Whether to freeze the entity embedding layer.')
    parser.add_argument('--ie_dim', default=ie_dim, type=int, help='number of the hidden units of the MInt operator.')
    parser.add_argument('--info_exchange', default=info_exchange, choices=[True, False, "every-other-layer"], type=utils.bool_str_flag, help="Whether we have the MInt operator in every GreaseLM layer or every other GreaseLM layer or not at all.")
    parser.add_argument('--ie_layer_num', default=ie_layer_num, type=int, help='number of hidden layers in the MInt operator')
    parser.add_argument("--sep_ie_layers", default=sep_ie_layers, type=utils.bool_flag, help="Whether to share parameters across the MInt ops across differernt GreaseLM layers or not. Setting it to `False` means sharing.")
    parser.add_argument('--random_ent_emb', default=random_ent_emb, type=utils.bool_flag, nargs='?', const=True, help='Whether to use randomly initialized learnable entity embeddings or not.')
    parser.add_argument("--cxt_node_connects_all", default=cxt_node_connects_all, type=utils.bool_flag, help="Whether to connect the interaction node to all the retrieved KG nodes or only the linked nodes.")
    ...
    # Optimization
    parser.add_argument('-dlr', '--decoder_lr', default=decoder_lr, type=float, help='Learning rate of parameters not in LM')
    parser.add_argument('-mbs', '--mini_batch_size', default=mini_batch_size, type=int)
    parser.add_argument('-ebs', '--eval_batch_size', default=eval_batch_size, type=int)
    parser.add_argument('--unfreeze_epoch', default=unfreeze_epoch, type=int, help="Number of the first few epochs in which LM’s parameters are kept frozen.")
    parser.add_argument('--refreeze_epoch', default=refreeze_epoch, type=int)
    parser.add_argument('--init_range', default=init_range, type=float, help='stddev when initializing with normal distribution')
