...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __init__(self, device0, device1, batch_size, indexes, qids, labels,
                 tensors0=[], lists0=[], tensors1=[], lists1=[], adj_data=None,true_labels=None):
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            # if self.true_labels:
            batch_true_labels = self._to_device(self.true_labels[batch_indexes], self.device1)
            ...
            yield tuple([batch_qids, batch_labels,batch_true_labels, *batch_tensors0, *batch_lists0, *batch_tensors1, *batch_lists1, edge_index, edge_type])
...
class GreaseLM_DataLoader(object):
    ...
    def __init__(self, train_statement_path, train_adj_path,
        ...
        _, self.train_true_labels, _, _ = self.load_input_tensors(train_statement_path, max_seq_length)
        ...
        *self.dev_decoder_data, self.dev_adj_data = self.load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, dev_concepts_by_sents_list)
        ...
        if subsample < 1. or n_train >= 0:
            ...
            if self.is_inhouse:
                ...
                self.train_true_labels = self.train_true_labels[:n_train]
    ...
    def train_size(self):
        c = self.inhouse_train_indexes.size(0) if self.is_inhouse else len(self.train_qids)
    ...
    def train(self):
        ...
        return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.batch_size, train_indexes, self.train_qids, self.train_labels,tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data,true_labels=self.train_true_labels)
    ...
    def dev(self):
        ...
        return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, dev_indexes, self.dev_qids, self.dev_labels, tensors0=self.dev_encoder_data, tensors1=self.dev_decoder_data, adj_data=self.dev_adj_data,true_labels=self.dev_labels)
    ...
    def test(self):
        ...
        if self.is_inhouse:
            return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, test_indexes, self.train_qids, self.train_labels, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data,true_labels=self.train_true_labels)
        else:
            return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, test_indexes, self.test_qids, self.test_labels, tensors0=self.test_encoder_data, tensors1=self.test_decoder_data, adj_data=self.test_adj_data,true_labels=self.test_labels)
    ...
    def load_resources(self, kg):
        ...
        tokenizer = tokenizer_class.from_pretrained(pretrained_model_name_or_path='/home/dell/.cache/huggingface/transformers/large/')
